...
import Network.HTTP.Types (status200, status404, status403, status500)
  ...
  case (portResult, tokenResult, dockerComposePathResult) of
    (Just port, Just token, Just dockerComposePath) ->
      case readMaybe port :: Maybe Int of
        ...
        Just portNum -> return $ AppConfig portNum (pack token) (pack dockerComposePath)
...
serverPort :: AppConfig -> Int
serverPort (AppConfig port _ _) = port

appToken :: AppConfig -> Text
appToken (AppConfig _ token _) = token

dockerComposePath :: AppConfig -> Text
dockerComposePath (AppConfig _ _ dcPath) = dcPath
...
getHTTPQueryParam query name =
    case join $ lookup name query of
        Nothing -> ""
        Just val -> decodeUtf8 val

runApp appName dockerComposePathValue = do
    (exitcode', stdout', stderr') <- readProcessWithExitCode appName [dockerComposePathValue] ""
    return exitcode'
...
hook req = do
    ...
    let applicationToken = appToken config :: Text
    let dockerComposePathValue = dockerComposePath config :: Text
    ...
    case tokenParam == applicationToken of
        True -> do
            ...
            let cmd = "docker-compose -f " ++ filename ++ " pull && docker-compose -f " ++ filename ++ " restart" ++ name
            code <- runApp cmd path
            case code of
                ExitSuccess -> return $ responseBuilder
                    status200
                    [(hContentType, "text/plain")]
                    $ fromString "Ok"
                _ -> return $ responseBuilder
                    status500
                    [(hContentType, "text/plain")]
                    $ fromString "err"
        False -> do
...
notFound = do
    return $ responseBuilder status404 [ ("Content-Type", "text/plain") ] $ mconcat $ map copyByteString
        [ "Not found" ]